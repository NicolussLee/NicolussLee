...
function addTopCart() {
	...
	$.ajax({
		...
		"success": function(response) {
            ...
            $("#shop_cart>ul").html("");
            $(".shop_cart span").text('');
            $(".shop_cart span").text(response.data.length);
            $("#cartList").html("");
            if(response.data.length>0){ $("#shop_cart").children("#noShop").hide();}else { $("#shop_cart").children("#noShop").show();}
			$('#cartList').html(html);
            $("#shop_cart").append('<button>去购物车结算</button>');
			$(".cart-icon .cart-count strong,.shop_cart strong").text(obj.length);
			$("#shop_cart button").click(function(){
...
function deletTopCart(){
	$(".all_carts_item").click(function(event) {
